...
class Ilib_Payment_Html_Provider_FakeQuickpay_PaymentProcess
{
    ...
    public function __construct($md5_secret) 
    {
    ...
    public function run($input)
    {
        ...
        if(isset($input['cardnum'])) {
            
            if(empty($_SESSION['payment_details'])) {
                trigger_error('Sessions are not working properly. They need to do!', E_USER_ERROR);
                exit;
            }
            
            
            if(empty($input['cardnum']) || empty($input['cvd'])) {
                header('location: '.$_SESSION['payment_details']['errorpage']);
                exit;
            }
            
            $time = date('ymdHis');
            $md5check = md5($_SESSION['payment_details']['amount'].$time.$_SESSION['payment_details']['ordernum'].'000'.'000'.'Approved'.'test@intraface.dk'.$_SESSION['payment_details']['merchant'].$_SESSION['payment_details']['currency'].'Visa'.'123'.$this->md5_secret);

            require_once 'HTTP/Request.php';
            $client = new HTTP_Request($_SESSION['payment_details']['resultpage']);
            $client->setMethod(HTTP_REQUEST_METHOD_POST);
            $client->AddPostData('amount', $_SESSION['payment_details']['amount']);
            $client->AddPostData('time', $time);
            $client->AddPostData('ordernum', $_SESSION['payment_details']['ordernum']);
            ...
            $client->AddPostData('merchant', $_SESSION['payment_details']['merchant']);
            $client->AddPostData('currency', $_SESSION['payment_details']['currency']);
            $client->AddPostData('cardtype', 'Visa');
            $client->AddPostData('transaction', '123');
            $client->AddPostData('md5checkV2', $md5check);
            
            foreach($_SESSION['payment_details'] AS $key => $value) {
            ...
            if(PEAR::isError($request)) {
                trigger_error('Error in post reguest: '.$request->getUserInfo(), E_USER_ERROR);
                exit;
            }
            ...
            if($result != 'SUCCESS!') {
                trigger_error('Error in processing the order. We got this message: '. $result, E_USER_ERROR);
                exit;
            } 
            
            header('location: '.$_SESSION['payment_details']['okpage']);
            exit;
        }
        else {
            ...
            foreach($required_fields AS $field) {
                if(!isset($input[$field])) {
                    trigger_error('The field '.$field.' need to be present!', E_USER_ERROR);
                    exit;
                }
            }            
            
            $_SESSION['payment_details'] = $input;
            ...
            if(!empty($input['ccipage'])) {
                ...
                $content = str_replace('https://secure.quickpay.dk/quickpay_pay.php', $_SERVER['PHP_SELF'], $content);
                
                echo '<div style="color: red;">TEST SERVER!</div>';
                
                echo $content;
                
            }
            else {
                trigger_error('At this moment this only works with CCI page!', E_USER_ERROR);
                exit;
            }
        }
    }
